...
// 17. 库删除与重建语义 + 表级提示 + 盘点去噪
let rOut = await run('removeDb', { db: 'tmp-del' })
check('removeDb 需 confirm', rOut.includes('confirm: true'), rOut)
rOut = await run('removeDb', { db: 'default', confirm: true })
check('removeDb 禁删默认库', rOut.includes('不允许删除默认库'), rOut)
await run('exec', { sql: 'CREATE TABLE t1 (id INTEGER)', db: 'tmp-del' })
rOut = await run('removeDb', { db: 'tmp-del', confirm: true })
check('removeDb 删除成功', rOut.includes('已删除库'), rOut)
check('removeDb 文件已移除', !existsSync(join(dir, 'tmp-del.db')))
const coord2 = createCoordination()
coord2.knownDbs = new Set(['tmp-del'])
onPostExecute(coord2, { name: 'sqlite_exec', arguments: { db: 'tmp-del' }, agent: agentA }, { isError: false })
onPostExecute(coord2, { name: 'sqlite_remove_db', arguments: { db: 'tmp-del' }, agent: agentA }, { isError: false })
check('onDbRemoved 清计数与已知库', coord2.dbCounters.has('tmp-del') === false && coord2.knownDbs.has('tmp-del') === false)
onPostExecute(coord2, { name: 'sqlite_exec', arguments: { db: 'tmp-del' }, agent: agentA }, { isError: false })
const t2 = preStepTexts(coord2, agentB, engine)
check('重建库重新广播', t2.some((x) => x.includes('tmp-del.db') && x.includes('新协作库')))
const coord3 = createCoordination()
coord3.knownDbs = new Set()
onPostExecute(coord3, { name: 'sqlite_exec', arguments: { db: 'hint-db', sql: 'CREATE TABLE orders (id INTEGER)' }, agent: agentA }, { isError: false })
onPostExecute(coord3, { name: 'sqlite_query', arguments: { db: 'hint-db' }, agent: agentB }, { isError: false })
onPostExecute(coord3, { name: 'sqlite_exec', arguments: { db: 'hint-db', sql: 'INSERT INTO orders VALUES (1)' }, agent: agentA }, { isError: false })
const t3 = preStepTexts(coord3, agentB, engine)
check('提醒含表级提示', t3.some((x) => x.includes('orders')))
check('盘点不再重复协作规则全文', !t3.some((x) => x.includes('协作规则：')))
