...
npm pack                                       # 产出 cxxl-dsh-terminal-0.1.7.tgz
dsh plugin --profile web add ./cxxl-dsh-terminal-0.1.7.tgz
```

## 使用

1. 在**视图切换栏**点 **`Terminal`** tab（位于「轨迹」右边）。
2. 终端作为会话视图占据主体区域，自动在**当前会话所在工作目录**（workspace 根）启动一个交互式 PowerShell。
